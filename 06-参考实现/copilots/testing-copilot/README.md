...
> AI 测试全能副驾驶 -- 编排测试策略、用例设计、测试执行、缺陷管理，覆盖 N10-N12 阶段
...
| 技能 | 用途 | 是否必须 |
|------|------|----------|
| test-case-generator（未安装） | 测试用例自动生成 | 否，缺失时使用内置方法降级 |
| test-automator（未安装） | 自动化测试脚本 | 否，缺失时提供手动步骤 |
| defect-analyzer（未安装） | 缺陷根因分析 | 否，缺失时人工分析模式 |
