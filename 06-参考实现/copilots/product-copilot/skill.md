...
**产物对齐（体系契约，QG-设计.12）**：Demo 产物必须对齐 [02-产物层/设计/产品原型设计规范.md](../../02-产物层/设计/产品原型设计规范.md) 的原型四件套——页面树 YAML（页面 ID + 5 项校验）、原型包（纯净无增强）、标注数据（ANN-NNN→REQ-NNN，需求标注覆盖率 100%，全量需求）、[原型说明](../../02-产物层/设计/原型说明模板.md)（需求↔页面覆盖矩阵）。ai-product-dev-kit-modular 的「标注数据.json / 页面树」产物结构与该规范一一对应。
...
- **QG-设计**（S 出口）：产品方案（覆盖矩阵 100%，第 11 项）+ 原型四件套（页面树校验/全量需求标注覆盖 100%/流程走通，第 12 项）+ PRD 原型引用一致（第 13 项）+ 架构/详设 齐全 + 评审通过
