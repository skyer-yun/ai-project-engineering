...
│   ├── pre-sales-copilot/             # → Spec-Writer 参考实现
│   ├── product-copilot/               # → Spec-Writer 参考实现
│   ├── dev-copilot/                   # → Builder 参考实现
│   ├── testing-copilot/               # → Shipper 参考实现
│   ├── delivery-copilot/              # → Shipper 参考实现
│   └── project-copilot/               # → Keeper 参考实现
└── skills/                            # 18 技能样例，按 DSBL 重组
    ├── spec/                          # D/S 阶段技能（需求/方案/PRD）
    ├── build/                         # S 后半/B 阶段技能（架构/详设/审查）
    ├── ship/                          # Ship 阶段技能（测试/部署/培训）
    └── learn/                         # 跨阶段/项目管理技能
```
...
| 五角色 | 参考实现 copilot | 备注 |
|--------|----------------|------|
| **Spec-Writer** | pre-sales-copilot + product-copilot | 售前+产品 合并示例 |
| **Builder** | dev-copilot | 研发 |
| **Reviewer** | （跨 copilot 抽取资深人/agent） | 无独立 copilot，由各 copilot 协作 |
| **Shipper** | testing-copilot + delivery-copilot | 测试+交付 合并示例 |
| **Keeper** | project-copilot | 项目管理 → Keeper |
...
- `code-reviewer`：代码审查（Reviewer 角色工具）
...
### learn/（跨阶段 + 项目管理）
- `quality-gate`：质量门禁检查（QG-D~Ship 落地）
- `risk-sentinel`：风险预警
...
- 用户反馈改进项录入 [02-产物层/L-Learn/项目复盘模板.md](../../02-产物层/L-Learn/项目复盘模板.md) 改进项区
