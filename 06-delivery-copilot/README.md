...
> AI 交付全能副驾驶 -- 编排部署实施、验收管理、交付文档生成，覆盖 N13-N15 阶段
...
| deployment-planner（待建） | 部署方案自动生成 | 否，缺失时使用内置模式 |
| acceptance-manager（待建） | 验收测试管理 | 否，缺失时使用内置流程 |
| delivery-doc-generator（待建） | 交付文档批量生成 | 否，缺失时使用内置模板 |
| pdf | 文档 PDF 输出 | 否 |
| xlsx | 验收清单/检查表 | 否 |
