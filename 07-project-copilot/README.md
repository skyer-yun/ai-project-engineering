...
> AI 项目管理全能副驾驶 -- 全生命周期管理 N1-N16，跨智能体协调，质量门禁

---

## 1. 这是什么

Project Copilot 是一个**项目管理智能体**，覆盖项目全生命周期（N1-N16），并作为总控协调其他 5 个智能体。
...
| dev-copilot | N8-N10 | 接收源码 + 接口文档 + 联调报告 |
| testing-copilot | N11-N12 | 接收测试报告 + 缺陷清单 |
...
N4-N7 产品阶段 → product-copilot
  ↓ QG2（需求/设计就绪）
N8-N10 开发阶段 → dev-copilot
  ↓ QG3（开发完成）
N11-N12 测试阶段 → testing-copilot
  ↓ QG4（测试通过）
...
| 交接点 | 产物 | 来源 → 目标 |
|--------|------|------------|
| QG1 | 需求确认书 + 技术方案 | pre-sales → product |
| QG2 | PRD + Demo + 设计规范 | product → dev |
| QG3 | 源码 + 接口文档 + 联调报告 | dev → testing |
| QG4 | 测试报告 + 缺陷清单(清零) | testing → delivery |
| QG5 | 交付文档包 + 验收报告 | delivery → project |
