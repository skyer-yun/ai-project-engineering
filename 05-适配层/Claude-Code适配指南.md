...
| **DSBL 五阶段产物** | Skill 引用 [02-产物层/](../02-产物层/) 模板，按阶段生成 |
| **Loop 微循环（P/A/O/R/E）** | Skill workflow 文件 + `Plan Mode` 触发 Plan/Reflect |
| **质量门禁 QG-D/S/B/Ship** | Skill 内置 checklist + `ExitPlanMode` Hook 强制审核 |
...
```
~/.claude/skills/
├── pre-sales-copilot/      # → Spec-Writer 参考
├── product-copilot/        # → Spec-Writer 参考
├── dev-copilot/            # → Builder 参考
├── testing-copilot/        # → Shipper 参考
├── delivery-copilot/       # → Shipper 参考
├── project-copilot/        # → Keeper 参考
...
|---------|----------------|---------|
| **Spec-Writer** | pre-sales-copilot（售前场景）/ product-copilot（产品场景） | solution-architect |
| **Builder** | dev-copilot | architecture-designer / detailed-designer / code-reviewer |
| **Reviewer** | code-reviewer skill（跨 copilot 抽取） | quality-gate skill |
| **Shipper** | testing-copilot + delivery-copilot | test-case-generator / auto-deployer |
| **Keeper** | project-copilot | risk-sentinel / smart-scheduler |

**关键**：1 角色 = 多 copilot 可承担；1 copilot = 可跨角色。Claude Code 会话内可切换角色。

---

## 四、典型 DSBL × Claude-Code 工作流
...
ExitPlanMode → 用户审批
  ↓
Act：填充 [02-产物层/D-Discover/需求分析报告模板.md]
  ↓
...
Plan Mode → Act：生成技术方案 / PRD / 架构设计
  ↓
Observe：QG-S checklist 自动检查
  ↓
Reflect：code-reviewer skill 介入（Reviewer 角色）
  ↓
...
- 不可逆性 L3/L4 节点（D 共识、S 方案、Ship 上线）**必须**用 Plan Mode
...
本技能执行时必须遵守：
- 阶段定义：[../../01-标准层/01-DSBL五阶段定义.md]
- Loop 微循环：[../../01-标准层/02-Loop微循环规范.md]
- 质量门禁：[../../01-标准层/03-质量门禁QG-D至QG-Ship.md]
...
## 输出产物（去下游 handoff）
按 [../../02-产物层/S-Spec/] 模板生成，写入 handoff.yaml。
```
...
A：不。最小化部署只装 1 个（如 dev-copilot 走 Builder 角色），其余靠 Claude Code 原生路由。
